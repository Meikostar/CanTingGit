 ...
 * 版本:
 ***/
public class LiveItemSelectBindDialog {
    ...
    public LiveItemSelectBindDialog(Context mContext, String s, String c, String x) {
    ...
    public interface BindClickListener{
        void site(int province, int city, int district);
    }
        ...
        mButtonConfirm.setOnClickListener(new View.OnClickListener() {//确定
            ...
            public void onClick(View v) {
                ...
                if(mBindClickListener!=null){
                    mBindClickListener.site(mCycleWheelViewProvince.getSelection(),mCycleWheelViewCity.getSelection(),
                            mCycleWheelViewDistrict.getSelection());
                }
            }
        });
  ...
  private DisMissListener listener;
    ...
    private void initNetDatas(){
        parseAddressData(LiveMineFragments.datas);
    }



    private void parseAddressData(List<LiveTypeBean> provinceList) {
        for(int i=0;i<provinceList.size();i++){
            if (!TextUtils.isEmpty(provinceList.get(i).firstName)&&provinceList.get(i).firstName.contains(mCurrentProviceName)){

                mCurrentProviceIndex = i;
            }
            mProvinceDatas.add(i,provinceList.get(i).firstName);
            List<LiveTypeBean> cityList = provinceList.get(i).secondList;
            List<String> cityNames = new ArrayList<>();
            if (cityList!=null && cityList.size()>0){
                for(int j=0;j<cityList.size();j++){
                    if (!TextUtils.isEmpty(cityList.get(j).secondName)&&cityList.get(j).secondName.contains(mCurrentCityName)){

                        mCurrentCityIndex = j;
                    }
                    cityNames.add(j,cityList.get(j).secondName);
                    List<LiveTypeBean> districtList = cityList.get(j).thirdList;
                    List<String> distrinctName = new ArrayList<>();
                    if (districtList!=null && districtList.size()>0){
                        for(int k=0;k<districtList.size();k++){
                            distrinctName.add(k,districtList.get(k).sec_category_name);
                            if (!TextUtils.isEmpty(districtList.get(k).sec_category_name)&&districtList.get(k).sec_category_name.contains(mCurrentDistrictName)){
                    ...
                    mDistrictDatasMap.put(cityList.get(j).secondName, distrinctName);
                }
            }
            // 省-市的数据，保存到mCitisDatasMap
            mCitisDatasMap.put(provinceList.get(i).firstName, cityNames);
        }
    }
