...
public class PraiseTextView extends TextView {
     ...
     * 第一个显示的图标
     */
    private int mIcon = R.drawable.qf3;
    /**
     * 名字文字颜色，分割文本用textview默认的，自行设置即可
     */
    private int mNameTextColor = R.color.slow_blacks;
    /**
     * 不设置默认与文字大小匹配
     */
    ...
    private SpannableStringBuilder getPraiseString () {
        SpannableStringBuilder mBuilder = new SpannableStringBuilder(getContext().getString(R.string.w));
    ...
    public class iconimage extends ImageSpan {
        ...
        public Drawable getDrawable () {
            if (mIconSize == null) {
                Rect mRect = new Rect();
                getPaint ().getTextBounds (getContext().getString(R.string.w), 0, 1, mRect);
