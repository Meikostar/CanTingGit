...
public class ChatRecordActivity extends BaseActivity1 implements ZhuYeViewCallBack {
    ...
    public void initViews() {
        setContentView(R.layout.activity_record_chats);
    ...
    public void getIntegral() {
        ...
        api.getChatIntegralDetail(map).enqueue(new BaseCallBack<INTEGRALIST>() {
            @Override
            public void onSuccess(INTEGRALIST bean) {
                if (bean != null && bean.data != null && bean.data.size() > 0) {
                    loadingView.showPager(LoadingPager.STATE_SUCCEED);
                    tvCout.setText(bean.userIntegralCount+"");
