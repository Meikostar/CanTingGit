...
public class ControlLiveViewLands extends RelativeLayout implements ViewAction, ITheme {

    private static final String TAG = ControlLiveViewLands.class.getSimpleName();
    ...
    public ControlLiveViewLands(Context context) {
        super(context);
        init();
        mContext=context;
    }

    public ControlLiveViewLands(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
        mContext=context;
    }

    public ControlLiveViewLands(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        //Inflate布局
        LayoutInflater.from(getContext()).inflate(R.layout.zhibozhubo, this, true);
    ...
    private void findAllViews() {
        ...
        mTitlebarBackBtn = findViewById(R.id.iv_close);
        ...
        mTitleMore = findViewById(R.id.iv_share);
        ll_care = findViewById(R.id.ll_care);


        et_comment = findViewById(R.id.iv_text);
        tv_care = findViewById(R.id.tv_cares);
        ...
        if(CanTingAppLication.landType==8){
    ...
    private void setViewListener() {
        ...
        ll_care.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                mOngiftMessageListener.onClick(3,state);
            }
        });
...
//标题的返回按钮监听
        ...
        et_comment.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (mOngiftMessageListener != null) {
                    mOngiftMessageListener.onClick(2,state);
                }
            }
        });
...
//            mTitlebarText.setText("");
    ...
    public void updateTitleView(String title) {
     if(TextUtil.isNotEmpty(title)){
         ...
         if(!split[0].equals("#")){
             Glide.with(mContext).load(split[0]).asBitmap().transform(new CircleTransform(mContext)).placeholder(R.drawable.editor_ava).into(iv_img);
         }
         if(!split[1].equals("#")){
           tv_name.setText(split[1]);
         }
         if(!split[2].equals("#")){
            tv_number.setText("粉丝数: "+split[2]);
         }
         if(!split[3].equals("#")){
             if(split[3].equals("1")){
...
//        mScreenModeBtn.setImageResource(R.drawable.alivc_screen_mode_small);
    ...
    private static class HideHandler extends Handler {
        private WeakReference<ControlLiveViewLands> controlViewWeakReference;

        public HideHandler(ControlLiveViewLands controlView) {
            controlViewWeakReference = new WeakReference<ControlLiveViewLands>(controlView);
        }

        @Override
        public void handleMessage(Message msg) {

            ControlLiveViewLands controlView = controlViewWeakReference.get();
            if (controlView != null) {
...
//        hideQualityDialog();
    ...
    public interface OnSendGiftMessageListener {
         ...
         * 返回按钮点击事件
         */
        void onClick(int poistion,int state);
    }
