...
import static com.zhongchuang.canting.utils.HxMessageUtils.UID;
    ...
    protected void saveCallRecord() {
        ...
        switch (callingState) {
            ...
            case VERSION_NOT_SAME:
                txtBody = new EMTextMessageBody(getString(R.string.Add_a_button_was_clicked));
                break;
            default:
        ...
        if (TextUtil.isNotEmpty(callExt)) {
            ...
            if (split != null && split.length > 0) {
                for (int i = 0; i < split.length; i++) {
                    if (i == 0) {
                        user.hx_fuid = split[0];
                    } else if (i == 1) {
                        user.hx_fname = split[1];
                    } else {
                        user.hx_favater = split[2];
                    }

                }
                chatmessages.setUserinfo(user);
            }
        ...
        EMMessage msg = HxMessageUtils.exMsg(message, chatmessage);
