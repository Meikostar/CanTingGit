package com.zhongchuang.canting.easeui.conference;
...
import com.zhongchuang.canting.easeui.widget.EasePageIndicator;
import com.zhongchuang.canting.utils.PhoneStateManager;
...
public class ConferenceActivity extends BaseActivity implements EMConferenceListener {
    ...
    private void init() {
        ...
        if (isCreator) {
            ...
            incomingCallView.setInviteInfo(String.format("%1$s 邀请您加入视频 …", inviter));
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_OVERLAY_PERMISSION && Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (Settings.canDrawOverlays(activity)) {
                ...
                Toast.makeText(activity,"当前App未被授予悬浮窗权限 :", Toast.LENGTH_SHORT).show();
            }
            return;
        }
    ...
    private void sendInviteMessage(String to, String extension) {
        ...
        final EMMessage message = EMMessage.createTxtSendMessage("快来加入会议" + " - " + conference.getConferenceId(), to);
