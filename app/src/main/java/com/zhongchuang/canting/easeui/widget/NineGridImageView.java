...
public class NineGridImageView<T> extends ViewGroup {
    ...
    private List<WebImageView> mImageViewList = new ArrayList<>();
    ...
    private void layoutChildrenView(){
        ...
        for(int i = 0; i < childrenCount; i++){
            WebImageView childrenView = (WebImageView)getChildAt(i);
            if(mAdapter != null){
     ...
     * @param position 位置
     */
    private WebImageView getImageView(final int position){
        if(position < mImageViewList.size()){
            return mImageViewList.get(position);
        }else{
            if(mAdapter != null){
                WebImageView imageView = mAdapter.generateImageView(getContext());
    ...
    public void setImageStr(String groupAvatar) {
        String[] split = groupAvatar.split(",,,");

        List<String> dataList = new ArrayList<>();
        for (int i = 0; i < split.length; i++) {
            dataList.add(split[i]);
        }
        setAdapter(new NineGridImageViewAdapter() {
            @Override
            protected void onDisplayImage(Context context, WebImageView
                    imageView,
                                          Object o) {
...
//                imageView.setImageWithURL(context, (String) o, R.drawable
//                        .ease_group_icon);
//                                MyBitmapUtils.display(imageView, (String) o, R.mipmap
//                                        .default_avatar);
            }
        });
        setImagesData(dataList);
    }
}
